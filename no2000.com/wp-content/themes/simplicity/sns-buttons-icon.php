...
<div class="sns-buttons sns-buttons-icon<?php echo $viral_class; ?>">
  ...
  <ul class="snsb clearfix snsbs">
    <?php if ( is_twitter_btn_visible() )://Twitterボタンを表示するか ?>
  	<li class="twitter-btn-icon"><a href="http://twitter.com/share?text=<?php echo urlencode( get_the_title() ); ?>&amp;url=<?php the_permalink() ?><?php echo get_twitter_via_param(); //ツイートにTwitter ID ?>" class="twitter-btn-icon-link" target="blank" rel="nofollow"><span class="social-icon icon-twitter"></span><span class="social-count twitter-count"><i class="fa fa-spinner fa-spin"></i></span></a></li>
    <?php endif; ?>
    <?php if ( is_facebook_btn_visible() )://Facebookボタンを表示するか ?>
  	<li class="facebook-btn-icon"><a href="https://www.facebook.com/sharer/sharer.php?u=<?php the_permalink() ?>&amp;t=<?php echo urlencode( get_the_title() ); ?>" class="facebook-btn-icon-link" target="blank" rel="nofollow"><span class="social-icon icon-facebook"></span><span class="social-count facebook-count"><i class="fa fa-spinner fa-spin"></i></span></a></li>
    <?php endif; ?>
...
?>
    <a href="http://b.hatena.ne.jp/add?mode=confirm&amp;url=<?php echo get_encoded_url(get_permalink()) ?>&amp;title=<?php echo get_encoded_title( trim(wp_title( '', false)) ); ?>" class="hatena-btn-icon-link" target="blank" rel="nofollow"><span class="social-icon icon-hatena"></span><span class="social-count hatebu-count"><i class="fa fa-spinner fa-spin"></i></span></a></li>
    <?php endif; ?>
    <?php if ( is_pocket_btn_visible() )://pocketボタンを表示するか ?>
  	<li class="pocket-btn-icon"><a href="https://getpocket.com/edit?url=<?php the_permalink() ?>" class="pocket-btn-icon-link" target="blank" rel="nofollow"><span class="social-icon icon-pocket"></span><span class="social-count pocket-count"><i class="fa fa-spinner fa-spin"></i></span></a></li>
    <?php endif; ?>
    <?php if ( is_line_btn_visible() )://LINEボタンを表示するか ?>
      <?php if (is_mobile()): //モバイルの時のみ表示?>
    	<li class="line-btn-icon"><a href="http://line.me/R/msg/text/?<?php the_title(); ?>%0D%0A<?php the_permalink(); ?>" class="line-btn-icon-link" target="blank" rel="nofollow"><span class="social-icon icon-line"></span></a></li>
      <?php endif; ?>
    <?php endif; ?>
    ...
    <a href="http://feedly.com/index.html#subscription%2Ffeed%2F<?php urlencode(bloginfo('rss2_url')); ?>" class="feedly-btn-icon-link" target="blank" rel="nofollow"><span class="social-icon icon-feedly"></span><span class="social-count feedly-count"><i class="fa fa-spinner fa-spin"></i></span></a></li>
